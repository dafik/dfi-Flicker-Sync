...
QueueItem.prototype.syncAlbum = function () {
    ...
    function onAlbumCreated(albumId) {
        var opt = {
            ...
            collection_id: that.options.fCollectionId(),
        };

        DfiFlicker(opt, DfiSync.configuration.conf, function (err, response) {
            that.options.fPhotoSetId = function () {
    ...
    DfiFlicker(opt, DfiSync.configuration.conf, function (err, response) {
        ...
        onAlbumCreated(response.photoset[0]['$'].id);
    }, this);
};
...
QueueItem.prototype.syncPhoto = function () {
    ...
    function assignToAlbum(err, response) {
        console.log('syncPhoto id: ' + that.id + ' up onuploaded');
        that.setProgress(10);

        var opt = {
            method: 'flickr.photosets.addPhoto',
            photoset_id: that.options.fPhotoSetId(),
