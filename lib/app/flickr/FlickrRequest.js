...
module.exports = function (opts, oauth_client, oauth_token, oauth_token_secret, callback, calbackThis, onRequestCallback, onRequestCallbackThis) {
    console.log(util.inspect(opts, {depth: 0}));

    send(opts, oauth_client, oauth_token, oauth_token_secret, function (err, req) {
        if (err) return callback.call(calbackThis, err);

        if (typeof onRequestCallback == "function") {
            onRequestCallback.call(onRequestCallbackThis, null, req);
        }

        req.on('response', function (res) {
            console.log(util.inspect(res.statusCode, {depth: 0}));
            callback.call(calbackThis, null, res);
        }).on('error', function (err) {
            req.destroy();
            callback.call(calbackThis, err);
        }).on('timeout', function () {
            req.destroy();
            callback.call(calbackThis, new Error('Timeout error'));
        }).setTimeout(30000);
    });
};
