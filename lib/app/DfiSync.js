            ...
            function doCompare() {
                ...
                local.forEach(function (lCollection) {
                    if (!hasItem(flickr, lCollection.title)) {
                        //mark green
                        var z = 2;
                        var t = localTree.fancytree('getTree')
                        //node.setExtraClasses('green')
                        var x = t.getNodeByKey(lCollection.key);
                        x.setSelected(true);
                        $(x.span).find('.fancytree-title').addClass('toSync');
                        ...
                        /*if (lCollection.hasOwnProperty('children') && lCollection.children.length > 0) {
                         ...
                         if (!hasPhoto(flickr[fCollectionKey].children[fPhotoSetKey].children, lPhoto.title)) {
                         //mark green
                         var x = 1;
                         }
                         });
