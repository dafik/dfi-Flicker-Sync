...
var Tree = require(global.libPath + '/app/tree/Tree');
var TreeCollection = require(global.libPath + '/app/tree/Collection');
var TreePhoto = require(global.libPath + '/app/tree/Photo');
...
var DfiSync = {
        ...
        aside: {
            ...
            onFlicker: function () {
                ...
                function onNavigate(e) {
                    that.parent.flickr.onNavigate.call(that.parent.flickr, e)
                }

                local.folder.on('navigate', onNavigate);
                local.addressBar.on('navigate', onNavigate);
        ...
        sync: {
            ...
            onQueue: function () {
                ...
                function onPlaceholderFound(err, placeholderId) {
                     ...
                     */
                    var nodeList = [];
                    localTree.getRootNode().visit(onNodeVisit)
                    /**
                     * @param {FancytreeNode} node
                     */
                    function onNodeVisit(node) {
                        var classes = node.extraClasses.split(' ');
                        if (node.selected || node.partsel || (-1 != classes.indexOf('fancytree-partsel'))) {
                            nodeList.push(node);
                        }
                    }

                    var tree = new Tree();
                    nodeList.forEach(buildSyncTree, that);
                    localTree.getRootNode().visit(clear)

                    /**
                     * @param {FancytreeNode} node
                     */
                    function buildSyncTree(node) {
                        var child;
                        if (node.data.ext != '') {
                            child = new TreePhoto(node);
                        } else {
                            child = new TreeCollection(node);
                        }
                        node.data.treeObj = child;
                        findParent(node).addChild(child);
                    }
                    ...
                    function findParent(node) {
                        if (node.parent.data.hasOwnProperty('treeObj')) {
                            return node.parent.data.treeObj;
                        }
                        return tree;
                    }

                    function clear(node) {
                        if (node.data.hasOwnProperty('treeObj')) {
                            delete node.data.treeObj;
                        }
                    }


                    tree.visit(buildQueues)
                     ...
                     */
                    function buildQueues(treeItem) {
                        if (treeItem.selected) {
                            switch (treeItem.data.level) {
                                case 1 :
                                    that.parent.queue.addCollection(treeItem);
                                    break;
                                case 2 :
                                    that.parent.queue.addAlbum(treeItem, placeholderId);
                                    break;
                                case 3 :
                                    that.parent.queue.addPhoto(treeItem, placeholderId);
                                    break;
                            }
            ...
            getTrees: function (callback, thisp) {
                if (this.treesLoaded) {
                    ...
                    function onLocalTree(err, results) {
                        ...
                        var partClass = 'fancytree-partsel';

                        function markParents(node) {
                            var parent = node.parent
                            if (parent.extraClasses == undefined) {
                                parent.extraClasses = partClass;
                            } else {
                                var classes = parent.extraClasses.split(' ');
                                classes.push(partClass);
                                classes = _.uniq(classes);
                                parent.extraClasses = classes.join(' ');
                                parent.partsel = true;
                            }
                            $(parent.span).addClass(partClass);
                            if (!parent.parent.isRoot()) {
                                markParents(parent)
                            }
                        }

                        function unmarkParents(node) {
                            var parent = node.parent
                            var hasChildrenSelected = false;
                            parent.children.forEach(function (childnode) {
                                if (childnode.isSelected() || $(childnode.span).hasClass(partClass)) {
                            ...
                            if (!hasChildrenSelected) {
                                if (parent.extraClasses != undefined) {
                                    var classes = parent.extraClasses.split(' ');
                                    if (classes.indexOf(partClass) != -1) {
                                        classes.splice(classes.indexOf(partClass), 1);
                                        parent.extraClasses = classes.join(' ');
                                    }
                                }
                                $(parent.span).removeClass(partClass);
                            }
                            if (!parent.parent.isRoot()) {
                                unmarkParents(parent)
                            }
                        }
                        ...
                        var treeOpt = {
                            ...
                            select: function (event, data) {
                                //console.log(util.inspect(event, {depth: 1}));
                                //console.log(util.inspect(data, {depth: 1}));
                                if (data.node.isSelected()) {
                                    if (!data.node.parent.isRoot()) {
                                        markParents(data.node)
                                    }
                                    selectChildren(data.node)
                                } else {
                                    if (!data.node.parent.isRoot()) {
                                        unmarkParents(data.node)
                                    }
                                    unSelectChildren(data.node)
                                }
        ...
        queue: {
            ...
            init: function (parent) {
                ...
                $('#clearAll').click(this.removeAll.bind(this));
                $('#clear').click(this.removeSelected.bind(this));
            ...
            insertItem: function (data) {
                ...
                switch (data.type) {
                    case QueueItem.Types.collection:
                        this.view.find('ul.collections').append(li).parent().show();
                        break;
                    case QueueItem.Types.album:
                        this.view.find('ul.albums').append(li).parent().show();
                        break;
                    case QueueItem.Types.photo:
                        this.view.find('ul.photos').append(li).parent().show();
                        break
                }
            ...
            removeSelected: function () {
                var that = this;
                this.view.find('input:checkbox:checked').each(function (item) {
                    item = $(this);
                    var type = item.data('type');
                    var id = item.data('id');
                    var queueKey = _.capitalize(type + 's');
                    /**
                     * @var Queue
                     */
                    var queue = that[queueKey];
                    queue.removeItem();
                })
            },

            /**
             * @param {FancytreeNode} treeItem
             * @param parentId
             */
            addCollection: function (treeItem) {
                var options = {
                    title: treeItem.data.name,
                    parentId: 0
                };

                if (treeItem.hasParent()) {

                    if (treeItem.parent.data.hasOwnProperty('fCollectionId')) {
                        options.parentId = treeItem.parent.data.fCollectionId;
                    } else {
                        treeItem.parent.addOnIdCallback(function (id) {
                            options.parentId = id;
                        })
                    }
                }
             ...
             * @param placeholderId
             */
            addAlbum: function (treeItem, placeholderId) {
                var options = {
                    title: treeItem.data.name,
                ...
                if (treeItem.parent.data.hasOwnProperty('fCollectionId')) {
                    options.collectionId = treeItem.parent.data.fCollectionId;
                } else {
                    treeItem.parent.addOnIdCallback(function (id) {
                        options.collectionId = id;
                    })
                }
             ...
             * @param {FancytreeNode} treeItem
             */
            addPhoto: function (treeItem, placeholderId) {
                var options = {
                    title: treeItem.data.name,
                    path: treeItem.data.path,
                    placeholderId: placeholderId,
                    userId: this.parent.configuration.getUserId(),
                    item: treeItem,
                    fAlbumId: undefined
                };
                if (treeItem.parent.data.hasOwnProperty('fAlbumId')) {
                    options.fAlbumId = treeItem.parent.data.fAlbumId;
                } else {
                    treeItem.parent.addOnIdCallback(function (id) {
                        options.fAlbumId = id;
                    });
        ...
        configuration: {
            ...
            init: function (parent) {
                ...
                if (typeof(this.conf.getConfig().app.folder) != "undefined") {
                    if (fs.existsSync(this.conf.getConfig().app.folder)) {
                        path = this.conf.getConfig().app.folder;
                    }
                }
