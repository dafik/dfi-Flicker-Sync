...
var DfiSync = {
        ...
        sync: {
            ...
            onQueue: function () {
                ...
                function onPlaceholderFound(err, placeholderId) {
                    ...
                    nodeList.forEach(onNode1, that);

                    /**
                     * @param {FancytreeNode} node
                     */
                    function onNode1(node) {
                        var child;
                        if (node.data.ext != '') {
                    ...
                    function onNode(node) {
                        if (node.extraClasses != undefined && -1 !== node.extraClasses.indexOf('fAdd')) {
                            switch (node.data.level) {
                                case 1 :
                                    this.parent.queue.addCollection(node, 0);
                                    break;
                                case 2 :
                                    this.parent.queue.addAlbum(node, placeholderId);
                                    break;
                                case 3 :
                                    this.parent.queue.addPhoto(node, placeholderId);
                                    break;
                            }
        ...
        queue: {
             ...
             * @param {FancytreeNode} node
             */
            addPhoto: function (node, placeholderId) {
                var options = {
                    title: node.title,
                    path: node.data.path,
                    placeholderId: placeholderId,
